...
TEST(dfgHash, HashCreator)
{
	...
	auto szHashMd5 = HashCreator(HashTypeMd5).process(szTest1, DFG_COUNTOF_CSL(szTest1)).toStr<char>();
	DFG_MODULE_NS(str)::toLower(szHashMd5);
	auto szHashSha1 = HashCreator(HashTypeSha1).process(szTest1, DFG_COUNTOF_CSL(szTest1)).toStr<char>();
